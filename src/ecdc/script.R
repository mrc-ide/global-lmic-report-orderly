...
message("Correcting an issue with Tanzanias vaccine data where it is reported as all 2 dose vaccinations but with only 1 dose numbers, please check this is still needed")
#split these assuming 50/50 ratio
who_vacc[who_vacc$ISO3 == "TZA",]$PERSONS_FULLY_VACCINATED <- round(who_vacc[who_vacc$ISO3 == "TZA",]$TOTAL_VACCINATIONS/3)
who_vacc[who_vacc$ISO3 == "TZA",]$PERSONS_VACCINATED_1PLUS_DOSE <- who_vacc[who_vacc$ISO3 == "TZA",]$TOTAL_VACCINATIONS - who_vacc[who_vacc$ISO3 == "TZA",]$PERSONS_FULLY_VACCINATED
